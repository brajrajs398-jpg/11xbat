...
import { slotSpin, diceRoll, plinkoDrop, wheelSpin, minesStart, minesReveal, minesCashout, coinFlipFlip, crashStart, crashCashout, crashStatus, limboBet, kenoDraw } from './games.js';
...
app.post('/api/wallet/change', requireAuth, async (req: AuthRequest, res) => {
  ...
  // NOTE: this endpoint still trusts the client for the remaining live-table
  // games (Roulette/Blackjack/Baccarat/Teen Patti/Game Show/Dice-table) that
  // have not yet been migrated to a server-authoritative endpoint like the
  // ones above. The cap below bounds the damage of a forged call to
  // realistic single-round winnings for those games, but does not close the
  // hole — only migrating each remaining engine the same way games.ts does
  // will. Bets (negative delta) are unrestricted since they only ever remove
  // money from the player, never create it.
