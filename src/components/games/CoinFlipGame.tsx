...
export default function CoinFlipGame({ onBack }: Props) {
  ...
  const flip = async () => {
    ...
    const finalRotation = rotation + spins * 360 + (outcome === 'tails' ? 180 : 0);
    ...
    setTimeout(async () => {
      ...
      if (isWin) {
        ...
        await recordHistory('coinflip', betAmount, payout, 1.96, { choice, result: outcome, won: true });
      } else {
        await recordHistory('coinflip', betAmount, 0, 0, { choice, result: outcome, won: false });
      }
      setFlipping(false);
    }, 2000);
  };
  ...
  return (
    <GameShell
      title="Coin Flip"
      description="Pick heads or tails and flip the coin for a 1.96× payout!"
      onBack={onBack}
