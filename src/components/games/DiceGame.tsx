...
export default function DiceGame({ onBack }: Props) {
  ...
  const handlePlay = async () => {
    ...
    if (isWin) {
      const payout = betAmount * multiplierPayout;
      await addWinnings(payout);
      await recordHistory('dice', betAmount, payout, multiplierPayout, { roll: result, target, isOver, won: true });
    } else {
      await recordHistory('dice', betAmount, 0, 0, { roll: result, target, isOver, won: false });
    }
  ...
  return (
    <GameShell
      title="Dice"
      description="Roll the dice and predict if the result will be over or under your target."
      onBack={onBack}
      currentPage="dice"
      controls={
        <BetControls
