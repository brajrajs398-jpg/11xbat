...
export default function PlinkoGame({ onBack }: Props) {
  ...
  const dropBall = async () => {
    ...
    const step = async () => {
      ...
      if (currentRow < ROWS) {
        ...
        const slot = Math.round((currentX / 100) * (MULTIPLIERS.length - 1));
        const m = MULTIPLIERS[Math.max(0, Math.min(MULTIPLIERS.length - 1, slot))];
        setBalls((b) => b.map((ball) => ball.id === id ? { ...ball, finished: true, slot } : ball));
        setLastResult({ slot, multiplier: m });

        const payout = betAmount * m;
        if (payout > 0) await addWinnings(payout);
        await recordHistory('plinko', betAmount, payout, m, { slot, multiplier: m });
