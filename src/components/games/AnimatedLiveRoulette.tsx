...
export default function AnimatedLiveRoulette({ game, onBack }: Props) {
  ...
  const handleSpin = () => {
    ...
    // Pick random outcome index
    const randomIndex = Math.floor(Math.random() * ROULETTE_NUMBERS.length);
    const resultNum = ROULETTE_NUMBERS[randomIndex];

    // Calculate rotation: 5 full spins (1800 deg) + offset for specific number
    ...
    setTimeout(() => {
      ...
      // Check win
      let isWin = false;
      let multiplier = 2; // Default for red/black/even/odd

      const isRed = RED_NUMBERS.includes(resultNum);
      const isBlack = resultNum !== 0 && !isRed;

      if (betType === 'red' && isRed) isWin = true;
      else if (betType === 'black' && isBlack) isWin = true;
      else if (betType === 'even' && resultNum !== 0 && resultNum % 2 === 0) isWin = true;
      else if (betType === 'odd' && resultNum % 2 !== 0) isWin = true;
      else if (betType === 'number' && resultNum === selectedNumber) {
        isWin = true;
        multiplier = 36;
      }
      ...
      setLastResult({ win: isWin, amount: winAmount, num: resultNum });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-amber-500/20 bg-gradient-to-b from-[#1c120c] via-[#120f0e] to-[#0a0807] p-6 text-center shadow-2xl">
