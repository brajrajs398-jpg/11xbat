...
export default function WheelGame({ onBack }: Props) {
  ...
  const spin = async () => {
    ...
    setTimeout(async () => {
      setResult({ multiplier: winningSegment.multiplier });
      const payout = betAmount * winningSegment.multiplier;
      if (payout > 0) await addWinnings(payout);
      await recordHistory('wheel', betAmount, payout, winningSegment.multiplier, { segment: winningIndex });
      setSpinning(false);
    }, 4000);
  };
